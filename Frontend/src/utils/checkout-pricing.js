...
export function paymentDiscountOnSpecialPrice(specialPrice, percent = CHECKOUT_DISCOUNT_PCT) {
...
export function resolveCheckoutPricing(quote) {
    ...
    const beforePaymentDiscount = quote.priceBeforePaymentDiscount ??
        (specialPrice > 0 ? specialPrice : 0);
    const paymentDiscountAmount = quote.paymentDiscountAmount ??
        (beforePaymentDiscount > 0
            ? paymentDiscountOnSpecialPrice(beforePaymentDiscount, paymentDiscountPercent).discountAmount
            : 0);
    const afterPaymentDiscount = quote.subtotalAfterPaymentDiscount ??
    ...
    return {
        ...
        walletCreditApplied,
        amountDue,
        totalSavings: listDiscountFromMrp +
            paymentDiscountAmount +
            referralCreditApplied +
            walletCreditApplied,
    };
