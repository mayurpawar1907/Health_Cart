...
import type { FamilyMember, UserProfile, WalletSummary } from '@/types'
...
export function ProfilePage() {
  ...
  const save = useMutation({
    ...
    onSuccess: async () => {
      ...
      if (nextName) dispatch(updateUser({ fullName: nextName }))
  ...
  return (
    <UserPage className="max-w-3xl">
      <UserPageHeader
        title="Profile"
        subtitle="Personal details, address, and preferences"
        actions={
          <Link to="/wallet">
      ...
      {wallet.data ? (
        <Link to="/wallet" className="block rounded-2xl border border-teal/20 bg-gradient-to-r from-teal-light/50 to-white p-4 transition hover:border-teal/40">
      ...
      <UserPanel title="Personal information">
        <div className="flex items-center gap-4">
          {form.avatarUrl ?? p.avatarUrl ? (
            <img src={form.avatarUrl ?? p.avatarUrl ?? ''} alt="" className="h-16 w-16 rounded-full bg-teal object-cover" />
          ) : (
            <span className="grid h-16 w-16 place-items-center rounded-full bg-teal text-xl font-bold text-white">
      ...
      <UserPanel title="Home collection address" subtitle="Required for free sample pickup">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {saveMsg ? (
            <p className={`text-sm ${saveMsg.includes('success') ? 'text-emerald-700' : 'text-brand-red'}`}>{saveMsg}</p>
          ) : null}
        </div>
      </UserPanel>
      ...
      <UserPanel title="Family members" subtitle="Manage members on your HealthID Card">
        <div className="space-y-2">
          {(family.data ?? []).map((f) => (
            <div key={f.id} className="flex items-center justify-between rounded-2xl border border-line px-4 py-3 text-sm">
      ...
      <UserPanel title="Security">
        ...
        {user.role === 'ADMIN' || user.role === 'SUPER_ADMIN' ? (
          <Link to="/admin" className="text-sm font-semibold text-teal hover:underline">Open operations console →</Link>
        ) : null}
      </UserPanel>
    </UserPage>
  )
...
function Sessions() {
  const q = useQuery({
    queryKey: ['sessions'],
    queryFn: async () => unwrap<{ id: string; userAgent?: string; createdAt: string }[]>((await api.get('/auth/sessions')).data),
  })
  if (!q.data?.length) return <p className="text-sm text-ink-soft">No other active sessions.</p>
  return (
    <div className="space-y-2">
      {q.data.map((s) => (
        <p key={s.id} className="text-sm text-ink-soft">{s.userAgent ?? 'Device'} · {new Date(s.createdAt).toLocaleString()}</p>
      ))}
    </div>
  )
}
