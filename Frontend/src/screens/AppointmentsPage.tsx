...
function statusTone(status: AppointmentStatus): 'teal' | 'success' | 'danger' | 'warn' | 'default' {
  if (status === 'COMPLETED') return 'success'
  if (status === 'CANCELLED') return 'danger'
  if (status === 'CONFIRMED') return 'teal'
  if (status === 'PENDING' || status === 'RESCHEDULED') return 'warn'
  return 'default'
}

function paymentTone(status?: string): 'success' | 'warn' | 'danger' | 'default' {
...
export function AppointmentsPage() {
  ...
  const q = useQuery({
    ...
    queryFn: async () => unwrap<Appointment[]>((await api.get('/appointments', { params: { status } })).data),
  })
  ...
  return (
    <UserPage>
      <UserPageHeader
        title="Bookings"
        subtitle="Track home collection appointments"
        actions={
      ...
      {q.isLoading ? (
        ...
        <EmptyState
          ...
          body="Book a laboratory test to see it here."
          action={{ label: 'Book a test', onClick: () => navigate('/appointments/book') }}
        />
      ) : (
        <UserTable>
          <table className="w-full min-w-[920px] text-left text-sm">
            ...
            <tbody>
              {!filtered.length ? (
                <UserEmptyRow colSpan={8} message="No bookings match your search" />
              ) : (
                filtered.map((a) => (
                  <UserTr key={a.id}>
...
export function AppointmentDetailsPage() {
  ...
  const q = useQuery({
    ...
    queryFn: async () => unwrap<Appointment>((await api.get(`/appointments/${id}`)).data),
  })
  ...
  if (q.isLoading || !q.data) return <Loading label="Loading booking" />
  const a = q.data
  ...
  return (
    <UserPage className="max-w-3xl">
      <UserPageHeader
        title={a.test.name}
        subtitle={`${a.code} · ${formatDate(a.date)} · ${a.timeSlot}`}
        actions={
      ...
      {a.status !== 'CANCELLED' && a.status !== 'COMPLETED' ? (
        <UserPanel title="Reschedule or cancel">
