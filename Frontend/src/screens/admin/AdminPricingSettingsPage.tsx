...
export function AdminPricingSettingsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => unwrap<PlatformPricingSettings>((await api.get('/admin/settings/pricing')).data),
  })

  const [form, setForm] = useState<PlatformPricingSettings | null>(null)
  ...
  const save = useMutation({
    mutationFn: async (payload: Partial<PlatformPricingSettings>) =>
      unwrap<PlatformPricingSettings>((await api.patch('/admin/settings/pricing', payload)).data),
    onSuccess: (updated) => {
      setForm(updated)
      setSaved(true)
  ...
  if (isLoading || !form) return <Loading label="Loading pricing settings" />
  ...
  return (
    <AdminPage>
      <AdminPageHeader
        title="Payment promo settings"
        subtitle="Manage checkout discount applied on special price (not MRP)"
        actions={
          <Button
        ...
        <AdminPanel title="Discount rules">
          <div className="space-y-5">
            <label className="flex items-center justify-between gap-4 rounded-2xl border border-line/60 bg-white/60 px-4 py-3">
              ...
              <input
                type="checkbox"
                checked={form.paymentPromoActive}
                onChange={(e) => setForm({ ...form, paymentPromoActive: e.target.checked })}
            ...
            <label className="flex items-center justify-between gap-4 rounded-2xl border border-line/60 bg-white/60 px-4 py-3">
              ...
              <input
                type="checkbox"
                checked={form.paymentPromoApplyToAllUsers}
                onChange={(e) => setForm({ ...form, paymentPromoApplyToAllUsers: e.target.checked })}
                className="h-5 w-5 rounded accent-teal"
              />
            </label>

            <Input
              label="Discount percent (on special price)"
              type="number"
              min={0}
              max={100}
              value={form.paymentPromoPercent}
              onChange={(e) => setForm({ ...form, paymentPromoPercent: Number(e.target.value) })}
            />

            <Input
              label="Promo label (shown in app)"
...
function SettingsPreview({ settings }: { settings: PlatformPricingSettings }) {
  ...
  return (
    <div className="rounded-2xl border border-[#e03a28]/20 bg-gradient-to-br from-[#fff5f4] to-teal-light/30 p-5">
      ...
      {settings.paymentPromoActive ? (
        <p className="mt-2 text-sm text-ink">
          Example: special price ₹{sampleSpecial.toLocaleString('en-IN')} → pay ~₹{after.toLocaleString('en-IN')} after {pct}% at checkout
        </p>
      ) : (
        <p className="mt-2 text-sm text-ink-soft">No payment discount will be applied while inactive.</p>
      )}
    </div>
  )
}
