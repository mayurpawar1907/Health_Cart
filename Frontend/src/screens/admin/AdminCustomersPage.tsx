...
type UserRow = {
  ...
  isActive: boolean
  createdAt: string
  _count: { appointments: number; memberships: number }
}

type UsersResponse = { items: UserRow[]; total: number; page: number; pages: number }
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () =>
      unwrap<UsersResponse>(
        (await api.get('/admin/users', { params: { q: q || undefined, page, role: roleFilter || undefined } })).data,
      ),
  })
                ...
                <AdminTr key={u.id}>
                  ...
                  <AdminTd>
                    {actor?.role === 'SUPER_ADMIN' && u.role !== 'SUPER_ADMIN' ? (
                      <AdminSelect value={u.role} onChange={(role) => setRole.mutate({ id: u.id, role })} className="text-xs py-1.5">
                        <option value="USER">USER</option>
                        <option value="ADMIN">ADMIN</option>
                        <option value="SUPER_ADMIN">SUPER_ADMIN</option>
                      </AdminSelect>
                    ) : (
                      <Badge tone={u.role === 'USER' ? 'default' : 'teal'}>{u.role.replace('_', ' ')}</Badge>
                    )}
                  </AdminTd>
