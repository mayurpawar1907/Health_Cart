...
export function AdminAppointmentsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () =>
      unwrap<Appt[]>((await api.get('/admin/appointments', { params: { q: q || undefined, status: status || undefined } })).data),
  })

  const setStatusMut = useMutation({
    mutationFn: ({ id, value }: { id: string; value: string }) => api.patch(`/admin/appointments/${id}/status`, { status: value }),
  ...
  const setPayment = useMutation({
    mutationFn: ({ id, paymentStatus }: { id: string; paymentStatus: string }) =>
      api.patch(`/admin/appointments/${id}/payment`, { paymentStatus }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['admin-appts'] }),
  })

  if (isLoading) return <Loading label="Loading appointments" />
    ...
    <AdminPage>
      ...
      <AdminTable>
        <table className="w-full min-w-[1000px] text-left text-sm">
          ...
          <tbody>
            {!data?.length ? (
              <EmptyRow colSpan={8} message="No appointments" />
            ) : (
              data.map((a) => (
                <AdminTr key={a.id}>
