...
export function AdminPaymentDetailPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    enabled: Boolean(id),
    queryFn: async () => unwrap<PaymentTransaction>((await api.get(`/admin/payments/${id}`)).data),
  })

  const updateStatus = useMutation({
    mutationFn: (status: string) => api.patch(`/admin/payments/${id}/status`, { status }),
    onSuccess: () => {
  ...
  return (
    <AdminPage>
      <AdminPageHeader
        ...
        subtitle={`${data.bookingCode} · ${data.testName}`}
        actions={
