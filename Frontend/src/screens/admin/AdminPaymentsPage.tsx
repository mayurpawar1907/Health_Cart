  ...
  return (
    <AdminPage>
      <AdminPageHeader
        ...
        subtitle="All booking payments, promo discounts, and settlement status"
      />
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard
          ...
          value={(data?.stats?.find((s) => s.status === 'FAILED')?.count ?? 0) + (data?.stats?.find((s) => s.status === 'REFUNDED')?.count ?? 0)}
        />
      </div>
      ...
      <AdminTable>
        <table className="w-full min-w-[1100px] text-left text-sm">
          ...
          <tbody>
            {!data?.items.length ? (
              <EmptyRow colSpan={9} message="No payment transactions" />
            ) : (
              data.items.map((t) => (
                <AdminTr key={t.id}>
