...
function specialPrice(mrp: number, discount: number) {
...
export function AdminTestsPage() {
  ...
  const tests = useQuery({
    ...
    queryFn: async () => unwrap<TestRow[]>((await api.get('/admin/tests', { params: { q: q || undefined } })).data),
  })
  const categories = useQuery({
    queryKey: ['admin-categories'],
    queryFn: async () => unwrap<Category[]>((await api.get('/admin/categories')).data),
  })

  const save = useMutation({
    mutationFn: async () => {
      const payload = { ...form, description: form.description || form.shortDescription }
      if (editing) return api.patch(`/admin/tests/${editing.id}`, payload)
  ...
  const toggleActive = useMutation({
    mutationFn: ({ id, isActive }: { id: string; isActive: boolean }) => api.patch(`/admin/tests/${id}`, { isActive }),
  ...
  if (tests.isLoading) return <Loading label="Loading tests" />

  return (
    <AdminPage>
      <AdminPageHeader
        title="Tests & rates"
        subtitle={`${filtered.length} tests in catalog`}
        actions={
          <Button
            size="sm"
      ...
      {(creating || editing) ? (
        <AdminPanel title={editing ? 'Edit test' : 'New test'} subtitle="Update MRP and HealthID Card special price">
      ...
      <AdminTable>
        <table className="w-full min-w-[900px] text-left text-sm">
          ...
          <tbody>
            {!filtered.length ? (
              <EmptyRow colSpan={6} message="No tests found" />
            ) : (
              filtered.map((t) => {
