...
import type { PaymentTransactionList } from '@/types'
...
function statusTone(status: string): 'success' | 'warn' | 'default' | 'danger' {
...
export function TransactionsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () =>
      unwrap<PaymentTransactionList>(
        (await api.get('/payments/transactions', { params: { page, limit: 15 } })).data,
      ),
  })
  ...
  return (
    <UserPage>
      <UserPageHeader
        ...
        subtitle="Lab booking payments, discounts applied, and settlement status"
        actions={
          <Link to="/wallet">
      ...
      <div>
        ...
        <UserTable>
          <table className="w-full min-w-[720px] text-left text-sm">
            ...
            <tbody>
              {items.length === 0 ? (
                <UserEmptyRow colSpan={7} message="No payment transactions yet. Book a test to see your payment ledger here." />
              ) : (
                items.map((t) => (
                  <UserTr key={t.id}>
                    ...
                    <UserTd>
                      ...
                      {t.breakdown?.paymentDiscountPercent > 0 ? (
                        <p className="text-[11px] text-[#e03a28]">
