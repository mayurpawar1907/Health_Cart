...
import type { FamilyMember, LabTest, PriceQuote, UserProfile } from '@/types'
...
export function BookAppointmentPage() {
  ...
  const [collectionType] = useState<'HOME' | 'LAB'>('HOME')
  ...
  const tests = useQuery({
    queryKey: ['tests', 'all'],
    queryFn: async () => unwrap<LabTest[]>((await api.get('/tests')).data),
    staleTime: 0,
  ...
  const slots = useQuery({
    queryKey: ['slots', date],
    enabled: !!date,
    queryFn: async () => unwrap<{ slots: string[] }>((await api.get('/appointments/slots', { params: { date } })).data),
  })
  const quote = useQuery({
    ...
    queryFn: async () =>
      unwrap<PriceQuote>(
        (await api.get('/appointments/quote', {
  ...
  function stepFromVisibleIndex(idx: number) {
  ...
  useEffect(() => {
    if (addr?.pincode) {
      api.get('/appointments/serviceability', { params: { pincode: addr.pincode } }).then((res) => {
        const data = unwrap<{ serviceable: boolean; message: string }>(res.data)
  ...
  if (tests.isLoading || profile.isLoading) return <Loading label="Preparing booking" />
  ...
  return (
    <div className="w-full space-y-5">
          ...
          {visibleSteps.map((label, idx) => {
            ...
            return (
              <button
                ...
                className={`flex flex-1 items-center gap-2 rounded-xl px-1 py-1 text-left transition ${
                  reachable ? 'cursor-pointer hover:bg-white/60' : 'cursor-not-allowed opacity-60'
                } ${current ? 'bg-white/50' : ''}`}
              >
                <div
                  className={`grid h-8 w-8 shrink-0 place-items-center rounded-xl text-xs font-bold transition ${
                    active ? 'bg-ink text-white' : 'bg-line/60 text-ink-soft'
                  } ${current ? 'ring-2 ring-teal/30 ring-offset-2' : ''}`}
                >
                  {n}
                </div>
      ...
      {/* Step 1 — Test table */}
      {step === 1 && tests.data ? (
        <BookTestTable tests={tests.data} onSelect={selectTest} />
      ) : null}
      ...
      {step === 2 && (
        <Card glass className="mx-auto w-full max-w-3xl space-y-5 p-5 md:p-6">
          ...
          {addr ? (
            <div className="rounded-2xl border border-line/80 bg-white/80 p-4">
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  {profile.data?.profile?.locationLat ? (
            ...
            <div className="rounded-2xl border border-brand-red/30 bg-brand-red-light/30 p-4">
                ...
                {(slots.data?.slots ?? []).map((s) => (
                  <button
                    ...
                    className={`rounded-xl border px-2 py-2.5 text-xs font-semibold transition ${
                      timeSlot === s ? 'border-teal bg-teal-light text-teal shadow-sm' : 'border-line hover:border-teal/40'
                    }`}
                  >
                    {s}
                  </button>
      ...
      {step === 3 && quote.data && selected ? (
        <Card glass className="mx-auto w-full max-w-3xl space-y-5 p-5 md:p-6">
          ...
          <BookPaymentPanel
            ...
            addressLine={addr ? `${addr.line1}, ${addr.city} — ${addr.pincode}` : undefined}
