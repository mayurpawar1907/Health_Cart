...
export function NotificationsPage() {
  ...
  const q = useQuery({
    ...
    queryFn: async () => unwrap<NotificationItem[]>((await api.get('/notifications')).data),
  })
  ...
  return (
    <UserPage>
      <UserPageHeader
        title="Notifications"
        subtitle="Appointment and report alerts"
        actions={
          q.data?.length ? (
            <Button variant="secondary" className="rounded-xl" onClick={() => readAll.mutate()}>
              Mark all read
            </Button>
          ) : null
        }
      ...
      {!q.data?.length ? (
        <EmptyState title="You're all caught up" body="Appointment and report alerts will appear here." />
      ) : (
        <UserTable>
          <table className="w-full min-w-[720px] text-left text-sm">
            ...
            <tbody>
              {q.data.map((n) => (
                <UserTr
                  key={n.id}
                  className={n.isRead ? undefined : 'bg-teal-light/20'}
                >
                  <UserTd>
