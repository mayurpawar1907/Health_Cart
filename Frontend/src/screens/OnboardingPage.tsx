...
export function OnboardingPage() {
  ...
  return (
    <div className="flex h-screen w-screen flex-col justify-between bg-cream px-8 py-10 lg:px-24">
      ...
      <div>
        <div className="mb-6 flex gap-2">
          {slides.map((_, idx) => (
            <span key={idx} className={`h-1.5 flex-1 rounded-full ${idx <= i ? 'bg-brand-red' : 'bg-line'}`} />
          ))}
        </div>
