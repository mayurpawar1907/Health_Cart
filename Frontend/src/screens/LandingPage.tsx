...
const STATS = [
  { value: `${PLATFORM.membership.flatDiscount}%`, label: 'Off at every payment' },
  { value: `${PLATFORM.testCount}+`, label: 'Blood tests & profiles' },
  { value: `${PLATFORM.packageCount}`, label: 'Health packages' },
  { value: `${PLATFORM.familyMembers}`, label: 'Family members per card' },
]
...
const BENEFITS = PLATFORM.membership.benefits
...
const TESTIMONIALS = [
  ...
  { name: 'Rahul M.', city: 'Bangalore', text: 'The free HealthID Card saved us 30% on every test. Adding my wife and kids to one card was seamless.', rating: 5 },
...
const FAQ = [
  { q: 'How does the extra 30% discount at payment work?', a: 'MRP is shown for reference only. You pay the special partner rate first, then get an extra flat 30% off that special price at checkout — not off MRP. Example: special price ₹200 → pay ₹140 after 30% discount.' },
  { q: 'Is the HealthID Card really free?', a: 'Yes. During our launch period, you get a 1-year family membership at zero cost — including exclusive special rates on 63+ tests and free home collection.' },
  { q: 'Which cities do you serve?', a: 'We offer home collection across Mumbai, Delhi NCR, Bangalore, Pune, Hyderabad, Chennai, and expanding rapidly.' },
  { q: 'How fast will I get my reports?', a: 'Most reports are available within 24–48 hours. You receive an in-app notification and a WhatsApp message when ready.' },
  { q: 'Can I book for family members?', a: 'Absolutely. Add up to 5 family members on your HealthID Card and book tests for anyone in your family.' },
]
    ...
    <div className="min-h-screen bg-cream text-ink">
        ...
        <div className="relative mx-auto grid max-w-6xl items-center gap-10 px-4 py-16 md:px-6 lg:grid-cols-2 lg:py-24">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            ...
            <h1 className="mt-5 font-display text-4xl leading-[1.1] tracking-tight md:text-5xl lg:text-[3.25rem]">
              Lab tests at home.<br />
              <span className="text-teal">One card for your whole family.</span>
            </h1>
            <p className="mt-5 max-w-lg text-lg leading-relaxed text-ink-soft">
              HealthID Card brings hospital-grade diagnostics to your doorstep — special rates on {PLATFORM.testCount}+ tests, an extra{' '}
              <strong className="text-ink">{PLATFORM.membership.flatDiscount}% off at every payment</strong>, WhatsApp reports, and a free 1-year family membership.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link to="/signup">
                <Button variant="accent" size="lg" className="rounded-xl px-8">Get your free card</Button>
              </Link>
              <Link to="/login">
                <Button variant="secondary" size="lg" className="rounded-xl px-8">Member login</Button>
              </Link>
            </div>
            ...
            <div className="overflow-hidden rounded-3xl border border-line/80 shadow-[0_24px_60px_rgba(43,90,121,0.15)]">
              ...
              <div className="bg-white p-6">
                ...
                <ul className="mt-4 space-y-2">
                  {[`Extra ${PLATFORM.membership.flatDiscount}% off at payment`, 'Special rates on 63+ tests', 'Free home collection', 'WhatsApp reports'].map((b) => (
        ...
        <div className="mx-auto grid max-w-6xl items-center gap-12 px-4 md:px-6 lg:grid-cols-2">
          <div>
            ...
            <p className="mt-4 text-white/75 leading-relaxed">
              One digital card. One year free. Exclusive partner rates on every test — for you, your spouse, parents, and children.
            </p>
            ...
            <Link to="/signup" className="mt-8 inline-block">
              <Button variant="accent" size="lg" className="rounded-xl px-8">Activate free card →</Button>
            </Link>
          </div>
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {TESTIMONIALS.map((t) => (
              <blockquote key={t.name} className="rounded-2xl border border-line bg-cream/50 p-6">
                ...
                <footer className="mt-4 text-sm font-semibold">{t.name} · <span className="font-normal text-ink-soft">{t.city}</span></footer>
              </blockquote>
            ))}
          </div>
        </div>
      ...
      <section className="bg-teal px-4 py-16 text-center text-white md:px-6">
        ...
        <p className="mx-auto mt-3 max-w-lg text-white/80">
          Free 1-year HealthID Card · Extra {PLATFORM.membership.flatDiscount}% off at payment · {PLATFORM.testCount}+ tests · Home collection
        </p>
        <div className="mt-8 flex flex-wrap justify-center gap-4">
          <Link to="/signup">
            <Button variant="accent" size="lg" className="rounded-xl px-10">Create free account</Button>
          </Link>
          <Link to="/login">
