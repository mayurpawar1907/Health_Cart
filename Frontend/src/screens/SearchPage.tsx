...
export function SearchPage() {
  ...
  const results = useQuery({
    ...
    queryFn: async () => unwrap<LabTest[]>((await api.get('/tests', { params: { search: debounced } })).data),
  })
    ...
    <UserPage>
      ...
      <UserToolbar>
        ...
        <button
          ...
          className="rounded-xl bg-teal px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-teal-dark"
        >
          Search
        </button>
      </UserToolbar>
          ...
          <div className="flex flex-wrap gap-2">
            {recent.map((r) => (
              <Link
                ...
                className="rounded-full border border-line/80 bg-white px-3 py-1 text-sm transition hover:border-teal/40 hover:text-teal"
              >
                {r}
              </Link>
            ))}
      ...
      {debounced.length <= 1 ? (
        <EmptyState title="Start searching" body="Try CBC, Vitamin D, thyroid, or a package name." />
      ) : results.isLoading ? (
        <Loading label="Searching tests" />
      ) : !results.data?.length ? (
        <EmptyState title="No matching tests" body="Try CBC, Vitamin D, thyroid or a package name." />
      ) : (
