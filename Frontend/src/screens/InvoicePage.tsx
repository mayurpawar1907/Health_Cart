...
export function InvoicePage({ mode }: InvoicePageProps) {
  const { id } = useParams<{ id: string }>()
  const printRef = useRef<HTMLDivElement>(null)

  const invoiceApi =
    mode === 'admin' ? `/admin/payments/${id}/invoice` : `/payments/transactions/${id}/invoice`
  const backTo = mode === 'admin' ? `/admin/payments/${id}` : `/transactions/${id}`

  const { data, isLoading, isError } = useQuery({
    queryKey: ['invoice', mode, id],
    enabled: Boolean(id),
    queryFn: async () => unwrap<PaymentInvoice>((await api.get(invoiceApi)).data),
  })
  ...
  return (
    <UserPage className="print:max-w-none print:p-0">
