...
type TestsCategoryGridProps = {
  categories: Category[]
  activeSlug: string
  onSelect: (slug: string) => void
}

export function TestsCategoryGrid({ categories, activeSlug, onSelect }: TestsCategoryGridProps) {
    ...
    <section aria-label="Browse by category">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <button
          ...
          onClick={() => onSelect('')}
          className={cn(
            'flex flex-col rounded-2xl border p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md',
            !activeSlug
        ...
        {categories.map((c, i) => {
          ...
          return (
            <button
              ...
              className={cn(
                'group flex flex-col rounded-2xl border p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md',
                active
                  ? 'border-teal/40 ring-2 ring-teal/20'
                  : 'border-line/60 hover:border-teal/30',
                `bg-gradient-to-br ${style.bg}`,
              )}
            >
              <span
                className={cn(
                  'mb-3 grid h-10 w-10 place-items-center rounded-xl shadow-sm transition group-hover:scale-105',
                  style.icon,
                )}
              >
