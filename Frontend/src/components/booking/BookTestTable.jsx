...
function SortHeader({ label, field, sortField, sortDir, onSort, align = 'left', }) {
    ...
    return (<button type="button" onClick={() => onSort(field)} className={cn('inline-flex items-center gap-1 transition hover:text-teal', align === 'right' && 'ml-auto', active ? 'text-teal' : 'text-ink-soft')}>
      {label}
      <Icon className={cn('h-3.5 w-3.5 shrink-0', !active && 'opacity-45')}/>
    </button>);
}
export function BookTestTable({ tests, onSelect }) {
    ...
    return (<Card glass className="w-full overflow-hidden p-0">
      <div className="border-b border-line/60 p-5 md:p-6">
        ...
        <div className="relative mt-4">
          ...
          <input type="search" value={query} onChange={(e) => onSearch(e.target.value)} placeholder="Search tests, packages, categories…" className="w-full rounded-xl border border-line/80 bg-white py-3 pl-10 pr-4 text-sm outline-none focus:border-teal focus:ring-2 focus:ring-teal/10"/>
        </div>
      </div>
      ...
      <div className="w-full overflow-x-auto">
        <table className="w-full table-fixed text-left text-sm">
          ...
          <tbody>
            ...
            return (<tr key={t.id} onClick={() => onSelect(t.id)} className="cursor-pointer border-b border-line/40 transition hover:bg-teal-light/40">
                    ...
                    <td className="px-4 py-3.5 md:px-6">
                      ...
                      <div className="mt-0.5 flex flex-wrap gap-1">
                        {t.isPackage ? (<span className="inline-block rounded-md bg-teal-light px-1.5 py-0.5 text-[10px] font-bold text-teal">
                            Package
                          </span>) : null}
                        {t.isPopular ? (<span className="inline-block rounded-md bg-amber-100 px-1.5 py-0.5 text-[10px] font-bold text-amber-800">
                            Popular
                          </span>) : null}
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3.5 text-right md:px-6">
                      <Button size="sm" variant="primary" className="rounded-xl text-xs" onClick={(e) => { e.stopPropagation(); onSelect(t.id); }}>
                        Select
                      </Button>
                    </td>
                  </tr>);
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-line/60 px-5 py-4 md:px-6">
        ...
        <div className="flex items-center gap-2">
          <Button size="sm" variant="secondary" className="rounded-xl" disabled={safePage <= 1} onClick={() => setPage((p) => Math.max(1, p - 1))}>
            <ChevronLeft className="h-4 w-4"/>
          </Button>
          <span className="min-w-[80px] text-center text-xs font-semibold text-ink">
            Page {safePage} / {totalPages}
          </span>
          <Button size="sm" variant="secondary" className="rounded-xl" disabled={safePage >= totalPages} onClick={() => setPage((p) => Math.min(totalPages, p + 1))}>
