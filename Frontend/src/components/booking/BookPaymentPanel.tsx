...
import type { LabTest, PriceQuote } from '@/types'
...
type BookPaymentPanelProps = {
  ...
  onUseReferral: (v: boolean) => void
  onPaymentMethod: (m: 'UPI' | 'CARD' | 'COD') => void
  onConfirm: () => void
  isPending: boolean
  error?: boolean
}
...
function PriceLine({
  ...
  label: string
  value: string
  tone?: 'default' | 'muted' | 'save' | 'accent' | 'wallet' | 'referral'
  bold?: boolean
  strike?: boolean
}) {
  const toneClass = {
...
export function BookPaymentPanel({
  selected,
  quote,
  date,
  timeSlot,
    ...
    <>
      ...
      <div className="rounded-2xl border border-teal/20 bg-gradient-to-br from-teal-light/60 to-white p-5">
        ...
        {!quote.isFreeForMember ? (
          <div className="mt-4 grid gap-2 rounded-xl border border-[#e03a28]/20 bg-[#fff8f7] p-3 sm:grid-cols-[1fr_auto_1fr_auto_1fr] sm:items-center sm:gap-3">
        ...
        <div className="mt-4 space-y-2.5 border-t border-teal/15 pt-4">
          ...
          {pricing.listDiscountFromMrp > 0 ? (
            <PriceLine
              label="MRP → special price savings"
              value={`−${formatMoney(pricing.listDiscountFromMrp)}`}
              tone="save"
            />
          ) : null}

          {pricing.beforePaymentDiscount > 0 && !quote.isFreeForMember ? (
            <PriceLine
              label="Special price (discount base)"
              value={formatMoney(pricing.beforePaymentDiscount)}
              bold
            />
          ) : null}

          {pricing.paymentDiscountAmount > 0 ? (
            <PriceLine
              label={`${pct}% off special price (${formatMoney(pricing.beforePaymentDiscount)} × ${pct}%)`}
              value={`−${formatMoney(pricing.paymentDiscountAmount)}`}
              tone="accent"
              bold
            />
          ) : null}
          ...
          {!quote.isFreeForMember ? (
            <PriceLine
              label="Test price after special-price discount"
              value={formatMoney(pricing.afterPaymentDiscount)}
              bold
            />
          ) : null}
          ...
          {pricing.referralCreditApplied > 0 ? (
            <PriceLine
              label={`Referral credit (max ₹${quote.referralPerTest}/test)`}
              value={`−${formatMoney(pricing.referralCreditApplied)}`}
              tone="referral"
            />
          ) : null}

          {pricing.walletCreditApplied > 0 ? (
            <PriceLine
              label="Wallet balance applied"
              value={`−${formatMoney(pricing.walletCreditApplied)}`}
              tone="wallet"
            />
          ) : null}
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {!quote.isFreeForMember ? (
            <Badge tone="success">{`${pct}% off special price applied`}</Badge>
          ) : null}
          {quote.membershipApplied ? <Badge tone="teal">Member rates active</Badge> : null}
        </div>
      </div>

      {!quote.isFreeForMember && (quote.walletBalance > 0 || quote.referralBalance > 0) ? (
        <div className="rounded-2xl border border-line/80 bg-white p-4">
          ...
          <div className="mt-3 space-y-3">
            {quote.referralBalance > 0 ? (
              <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-line/60 p-3 has-[:checked]:border-teal has-[:checked]:bg-teal-light/30">
                <input
                  type="checkbox"
                  checked={useReferral}
                  onChange={(e) => onUseReferral(e.target.checked)}
            ...
            {quote.walletBalance > 0 ? (
              <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-line/60 p-3 has-[:checked]:border-teal has-[:checked]:bg-teal-light/30">
                <input
                  type="checkbox"
                  checked={useWallet}
                  onChange={(e) => onUseWallet(e.target.checked)}
                  className="mt-1 accent-teal"
                />
                <div>
      ...
      {amountDue === 0 ? (
        ...
        <>
            ...
            <div className="grid grid-cols-3 gap-2">
              {(['UPI', 'CARD', 'COD'] as const).map((m) => (
                <button
                  key={m}
                  type="button"
                  onClick={() => { onPaymentMethod(m); setPaidSimulated(false); setUpiApp(null) }}
          ...
          {paymentMethod === 'UPI' ? (
            <div className="rounded-2xl border border-line/80 bg-white/80 p-4 md:p-5">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2">
                    {UPI_APPS.map((app) => (
                      <button
                        key={app.id}
                        type="button"
                        onClick={() => { setUpiApp(app.id); setPaidSimulated(true) }}
                        className={`flex items-center gap-2 rounded-xl border px-3 py-2.5 text-xs font-semibold ${
                          upiApp === app.id ? 'border-teal bg-teal-light text-teal' : 'border-line'
                        }`}
                      >
                        {app.name}
                      </button>
          ...
          {paymentMethod === 'COD' ? (
            <div className="rounded-2xl border border-teal/20 bg-teal-light/30 p-4 text-sm">
