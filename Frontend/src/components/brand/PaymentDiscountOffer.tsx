...
/** Fallback when API hasn't loaded — keeps SSR/build stable */
export const CHECKOUT_DISCOUNT_PCT = PLATFORM.membership.flatDiscount
...
export function PaymentDiscountBanner({ className }: { className?: string }) {
  const copy = usePaymentDiscountCopy()
  if (!copy.isPromoActive) return null

  return (
    <div
      className={cn(
        'relative overflow-hidden rounded-2xl border-2 border-[#e03a28]/25 bg-gradient-to-r from-[#fff0ee] via-white to-teal-light/40 px-4 py-4 md:px-6 md:py-5',
        className,
      )}
    >
...
export function PaymentDiscountStrip({ className }: { className?: string }) {
  const copy = usePaymentDiscountCopy()
  if (!copy.isPromoActive) return null
  ...
  return (
    <div
      className={cn(
        'flex flex-wrap items-center justify-center gap-2 bg-gradient-to-r from-[#e03a28] via-[#d43325] to-teal px-4 py-2.5 text-center text-sm font-semibold text-white',
        className,
      )}
    >
...
export function PaymentDiscountBadge({
  size = 'md',
  className,
}: {
  size?: 'sm' | 'md' | 'lg'
  ...
  return (
    <span
      className={cn(
        'inline-flex shrink-0 items-center gap-1 rounded-full bg-[#e03a28] font-bold uppercase tracking-wide text-white shadow-sm',
        sizes[size],
        className,
      )}
      title={copy.long}
...
export function PaymentDiscountCardNote({ specialPrice }: { specialPrice?: number }) {
  const copy = usePaymentDiscountCopy()
  if (!copy.isPromoActive) return null
  ...
  return (
    <div className="mt-2 rounded-lg border border-[#e03a28]/20 bg-[#fff5f4] px-2.5 py-1.5">
      <div className="flex items-start gap-2">
        ...
        <div>
          ...
          {after != null ? (
            <p className="mt-0.5 text-[10px] font-bold text-teal">
...
export function PaymentDiscountActivateLink() {
  return (
    <Link to="/home?edit=card" className="text-xs font-semibold text-teal hover:underline">
