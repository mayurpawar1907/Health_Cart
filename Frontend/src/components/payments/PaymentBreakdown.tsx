import type { PaymentTransactionBreakdown } from '@/types'
...
function SummaryCard({
  ...
  value: string
  hint?: string
  tone?: 'default' | 'accent' | 'save' | 'final'
}) {
  const tones = {
...
export function PaymentBreakdownSummary({ breakdown }: { breakdown: PaymentTransactionBreakdown }) {
  const totalDiscount =
    breakdown.listDiscountFromMrp +
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <SummaryCard
        ...
        value={breakdown.isFreeForMember ? 'FREE (member)' : `−${formatMoney(totalDiscount)}`}
        hint={
          breakdown.paymentDiscountPercent > 0
            ? `Includes ${breakdown.paymentDiscountPercent}% payment promo on special price`
            : 'Card + wallet + referral savings'
        }
        tone="save"
      />
      <SummaryCard
        label="Final paid"
        value={breakdown.isFreeForMember ? '₹0' : formatMoney(breakdown.amountDue)}
        hint="Amount settled at checkout"
        tone="final"
      />
    </div>
  )
...
export function PaymentBreakdown({ breakdown }: { breakdown: PaymentTransactionBreakdown }) {
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-line/60 bg-white/70 p-4 md:p-5">
        ...
        <div className="mt-3 divide-y divide-line/40">
          ...
          {breakdown.listDiscountFromMrp > 0 ? (
            <Line label="Discount vs MRP" value={`−${formatMoney(breakdown.listDiscountFromMrp)}`} tone="save" />
          ) : null}
          {!isFree && breakdown.paymentDiscountPercent > 0 ? (
            <>
              <Line
                label={`Payment promo (${breakdown.paymentDiscountPercent}% on special price)`}
                value={`−${formatMoney(breakdown.paymentDiscountAmount)}`}
                tone="accent"
              />
              <Line label="After payment discount" value={formatMoney(breakdown.subtotalAfterPaymentDiscount)} />
            </>
          ) : null}
          {breakdown.referralCreditApplied > 0 ? (
            <Line label="Referral credit" value={`−${formatMoney(breakdown.referralCreditApplied)}`} tone="referral" />
          ) : null}
          {breakdown.walletCreditApplied > 0 ? (
            <Line label="Wallet credit" value={`−${formatMoney(breakdown.walletCreditApplied)}`} tone="wallet" />
          ) : null}
          <Line
            label={isFree ? 'Member benefit' : 'Final paid amount'}
            value={isFree ? 'FREE' : formatMoney(breakdown.amountDue)}
            bold
          />
        </div>
      </div>
