...
export const InvoiceDocument = forwardRef<HTMLDivElement, InvoiceDocumentProps>(function InvoiceDocument(
  ...
  return (
    <div
      ref={ref}
      className="mx-auto max-w-3xl rounded-3xl border border-line/70 bg-white p-6 shadow-sm md:p-10 print:max-w-none print:rounded-none print:border-0 print:shadow-none"
    >
      <div className="flex flex-wrap items-start justify-between gap-6 border-b border-line/60 pb-6">
        ...
        <div className="text-right">
          ...
          {variant === 'admin' ? (
            <p className="mt-1 text-[10px] uppercase tracking-wide text-ink-soft">Operations copy</p>
          ) : null}
        </div>
      </div>
      ...
      <div className="mt-6 ml-auto max-w-sm space-y-2 text-sm">
        ...
        {p.paymentDiscountAmount > 0 ? (
          <Row
            label={`Payment promo (${p.paymentDiscountPercent}% on special price)`}
            value={`−${formatMoney(p.paymentDiscountAmount)}`}
            accent
          />
        ) : null}
        {p.referralCreditApplied > 0 ? (
          <Row label="Referral credit" value={`−${formatMoney(p.referralCreditApplied)}`} />
        ) : null}
