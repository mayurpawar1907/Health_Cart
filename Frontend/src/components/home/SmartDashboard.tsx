...
export function HomeExplore({ categories, popular }: SmartDashboardProps) {
  return (
            ...
            {categories.slice(0, 6).map((c, i) => {
              ...
              return (
                <Link
                  ...
                  className={cn(
                    'group flex flex-col rounded-2xl border border-line/60 bg-gradient-to-br p-4 shadow-sm transition hover:-translate-y-0.5 hover:border-teal/30 hover:shadow-md',
                    style.bg,
                  )}
                >
                  ...
                  {c.description ? (
                    <span className="mt-1 line-clamp-2 text-[11px] text-ink-soft">{c.description}</span>
                  ) : null}
                </Link>
              )
            })}
      ...
      {popular.length > 0 ? (
        <section>
          ...
          <div className="-mx-1 flex snap-x snap-mandatory gap-5 overflow-x-auto pb-3 scroll-smooth [scrollbar-width:thin]">
            {popular.filter((t) => !t.isPackage).slice(0, 8).map((t, i) => (
...
export function PopularTestRow({ test }: { test: LabTest }) {
  return (
    <Link
      to={`/appointments/book?testId=${test.id}`}
      className="flex items-center justify-between gap-3 rounded-2xl border border-line/60 bg-white px-4 py-3 transition hover:border-teal/30"
    >
