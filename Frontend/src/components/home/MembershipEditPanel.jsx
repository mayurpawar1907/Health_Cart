...
export function MembershipEditPanel({ onClose }) {
    ...
    if (membership.isLoading || plans.isLoading)
        return <Loading label="Loading card settings"/>;
    const m = membership.data;
    const plan = plans.data?.[0];
    if (!m) {
        return (<UserPanel title="Activate HealthID Card" subtitle="Free 1-year membership">
        <p className="text-sm text-ink-soft">{plan?.description}</p>
        {plan ? (<Button className="mt-4 rounded-xl" disabled={subscribe.isPending} onClick={() => subscribe.mutate(plan.id)}>
            Activate free card
          </Button>) : null}
    ...
    return (<div className="space-y-4">
      ...
      <UserPanel title="Family on your card" subtitle={`Up to ${m.plan.maxFamilyMembers ?? 5} members`}>
        <div className="space-y-2">
          {(m.members ?? []).map((member) => (<div key={member.id} className="rounded-xl border border-line/60 bg-white/60 px-4 py-3">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-4">
          ...
          <div className="flex items-end">
            <Button className="w-full rounded-xl" disabled={!name || addFamily.isPending} onClick={() => addFamily.mutate({ name, relation, age: age ? Number(age) : undefined })}>
              Add member
            </Button>
          </div>
        </div>
        {(family.data ?? []).length > (m.members?.length ?? 0) ? (<p className="mt-3 text-xs text-ink-soft">Saved family members sync to your card automatically.</p>) : null}
