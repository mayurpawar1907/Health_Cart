...
export function HomeActivatePanel({ onActivate, activatePending, compact }: HomeActivatePanelProps) {
  if (compact) {
    return (
      <div className="mx-auto w-full max-w-[400px] space-y-4 rounded-2xl border border-line/70 bg-white p-4 shadow-sm">
        <div>
        ...
        <ul className="grid gap-2 sm:grid-cols-2">
          {perks.slice(0, 4).map((item) => (
            <li key={item.text} className="flex items-center gap-2 rounded-lg bg-slate-50 px-2.5 py-2 text-xs text-ink-soft">
        ...
        {onActivate ? (
          <Button
            className="h-11 w-full rounded-xl border-0 bg-gradient-to-r from-[#e03a28] to-[#c42e1e] text-sm font-bold shadow-sm hover:brightness-105"
  ...
  return (
    <div className="flex h-full flex-col rounded-3xl border border-line/70 bg-white p-5 shadow-[0_8px_30px_rgba(12,25,41,0.06)] md:p-6">
      ...
      <ul className="mt-5 space-y-3">
        {perks.map((item) => (
          <li key={item.text} className="flex items-center gap-3 text-sm text-ink-soft">
      ...
      {onActivate ? (
        <Button
          className="mt-auto w-full rounded-xl border-0 bg-gradient-to-r from-[#e03a28] to-[#c42e1e] shadow-sm hover:brightness-105"
          size="lg"
          disabled={activatePending}
          onClick={onActivate}
        >
