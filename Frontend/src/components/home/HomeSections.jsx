...
import { Activity, Award, CalendarCheck, ChevronDown, ChevronRight, Droplets, FlaskConical, Heart, Home, MessageCircle, Microscope, Pill, ShieldCheck, Stethoscope, Truck, Users, } from 'lucide-react';
import { Button } from '@/components/ui/Button';
import { Input } from '@/components/ui/Input';
import { HealthIdCard } from '@/components/brand/HealthIdCard';
...
export function HeroSection({ userName, city, tests, mobile, }) {
    ...
    return (<section className="overflow-hidden rounded-2xl bg-gradient-to-br from-teal-light via-white to-[#fff8e6]">
        ...
        <div className="rounded-2xl border border-line bg-white p-6 shadow-[0_12px_40px_rgba(43,90,121,0.08)]">
          ...
          <form onSubmit={onBook} className="mt-5 space-y-3">
            ...
            <label className="block text-sm">
              Select test / package
              <select className="mt-1 w-full rounded-2xl border border-line px-4 py-3 text-sm outline-none focus:border-teal" value={testId} onChange={(e) => setTestId(e.target.value)}>
                <option value="">Choose a test</option>
                {tests.slice(0, 12).map((t) => (<option key={t.id} value={t.id}>{t.name}</option>))}
              </select>
            </label>
    ...
    return (<section>
      ...
      <div className="flex gap-4 overflow-x-auto pb-2 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {categories.map((c) => {
            ...
            return (<Link key={c.id} to={`/tests?category=${c.slug}`} className="flex min-w-[88px] flex-col items-center gap-2 rounded-xl border border-line bg-white px-4 py-4 text-center transition hover:border-teal hover:bg-teal-light/40">
...
export function HorizontalOffers({ children }) {
    return (<div className="flex gap-4 overflow-x-auto pb-3 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      {children}
    </div>);
}
export function MembershipSection({ hasMembership, cardData, }) {
    return (<section className="overflow-hidden rounded-2xl bg-gradient-to-r from-[#fff8e6] via-[#fff3d6] to-teal-light/50">
      <div className="grid items-center gap-8 p-6 lg:grid-cols-2 lg:p-8">
        <div>
          ...
          <ul className="mt-5 space-y-2.5 text-sm">
            {['Exclusive rates on 63+ tests', 'Add up to 5 family members', 'Free home sample collection', 'Reports on app + WhatsApp'].map((b) => (<li key={b} className="flex items-center gap-2">
        ...
        <div className="flex justify-center lg:justify-end">
          {hasMembership && cardData ? (<div className="scale-[0.92] origin-center lg:origin-right">
              <HealthIdCard data={cardData}/>
            </div>) : (<div className="w-full max-w-[380px] rounded-2xl border-2 border-dashed border-teal/30 bg-white/70 p-8 text-center">
...
function CreditCardPlaceholder() {
    return (<div className="mx-auto h-44 w-full max-w-[320px] rounded-2xl bg-gradient-to-br from-ink to-teal p-5 text-left text-white shadow-lg">
...
export function HowItWorksSection() {
    ...
    return (<section className="rounded-2xl border border-line bg-white p-6 md:p-8">
      ...
      <div className="mt-8 grid gap-6 md:grid-cols-3">
        {steps.map((s, i) => (<div key={s.title} className="relative text-center">
            {i < steps.length - 1 ? (<div className="absolute left-[calc(50%+40px)] top-8 hidden h-px w-[calc(100%-80px)] bg-line md:block"/>) : null}
...
export function TrustSection() {
    ...
    return (<section className="rounded-2xl bg-ink px-6 py-8 text-white md:px-10">
      <div className="flex flex-col items-center gap-6 md:flex-row md:justify-between">
        ...
        <div className="flex flex-wrap justify-center gap-3">
          {badges.map((b) => (<span key={b} className="rounded-lg border border-white/15 bg-white/10 px-4 py-2 text-xs font-medium uppercase tracking-wide text-white/85">
              {b}
            </span>))}
        </div>
      </div>
...
export function UpcomingStrip({ testName, date, time, address, href, }) {
    return (<div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-teal/20 bg-teal-light/60 px-5 py-4">
...
export function ComparisonTable({ packages }) {
    if (!packages.length)
        return null;
    const rows = ['Home collection', 'Digital reports', 'WhatsApp updates', 'Member 30% off', 'Family coverage'];
    return (<section className="overflow-hidden rounded-2xl border border-line bg-white">
        ...
        <table className="w-full min-w-[640px] text-left text-sm">
          <thead>
            <tr className="border-b border-line bg-cream/80">
              ...
              {packages.slice(0, 3).map((p) => (<th key={p.id} className="px-4 py-3 font-display text-base text-teal-dark">{p.name.replace(' Package', '').replace(' Checkup', '')}</th>))}
            </tr>
          </thead>
          <tbody>
            <tr className="border-b border-line">
              <td className="px-6 py-3 text-ink-soft">Price (member)</td>
              {packages.slice(0, 3).map((p) => (<td key={p.id} className="px-4 py-3 font-semibold">{formatMoney(p.memberPrice ?? p.discountedPrice ?? p.price)}</td>))}
            </tr>
            {rows.map((row) => (<tr key={row} className="border-b border-line last:border-0">
                <td className="px-6 py-3 text-ink-soft">{row}</td>
                {packages.slice(0, 3).map((p) => (<td key={p.id} className="px-4 py-3 text-success">✓ Included</td>))}
              </tr>))}
          </tbody>
        </table>
      </div>
