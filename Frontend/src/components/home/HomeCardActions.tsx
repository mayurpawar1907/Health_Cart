...
import type { HealthIdCardData } from '@/components/brand/HealthIdCard'
import { formatDate } from '@/lib/utils'

type HomeCardActionsProps = {
  cardData: HealthIdCardData
  flipped: boolean
  onFlip: () => void
  onEdit: () => void
  hasAddress: boolean
...
function ToolbarBtn({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`inline-flex flex-1 items-center justify-center gap-1.5 rounded-xl px-2 py-2.5 text-xs font-semibold transition ${
        highlight
...
export function HomeCardActions({ cardData, flipped, onFlip, onEdit, hasAddress }: HomeCardActionsProps) {
  return (
    <div className="mx-auto w-full max-w-[400px] space-y-4">
      ...
      <div className="flex gap-2">
        ...
        <ToolbarBtn
          ...
          icon={Share2}
          onClick={() =>
            navigator.share?.({ title: 'HealthID Card', text: `${cardData.memberName} · ${cardData.membershipId}` }).catch(() =>
              navigator.clipboard.writeText(cardData.membershipId),
            )
          }
        />
      ...
      {!hasAddress ? (
        <Link
          to="/profile"
          className="flex items-center gap-2.5 rounded-xl border border-amber-200/80 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-900 transition hover:bg-amber-100/70"
        >
