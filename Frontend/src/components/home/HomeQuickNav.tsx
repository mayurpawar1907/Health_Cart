...
export function HomeQuickNav() {
  return (
    <nav aria-label="Quick navigation" className="grid grid-cols-5 gap-2 sm:gap-3">
      {items.map((item) => {
        ...
        return (
          <Link
            key={item.to}
            to={item.to}
            className="flex flex-col items-center gap-2.5 rounded-2xl border border-line/70 bg-white px-2 py-4 text-center shadow-sm transition hover:-translate-y-0.5 hover:border-teal/25 hover:shadow-md"
          >
