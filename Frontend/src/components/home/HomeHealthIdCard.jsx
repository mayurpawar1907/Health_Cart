...
export function HomeHealthIdCard({ hasMembership, editOpen, onEditOpen, hasAddress }) {
    ...
    return (<section id="healthid-card-section" aria-labelledby="healthid-card-heading" className="overflow-hidden rounded-[28px] border border-line/80 bg-white shadow-[0_16px_48px_rgba(12,25,41,0.08)]">
      {/* Section header */}
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-line/60 bg-gradient-to-r from-teal-light/40 via-white to-[#fff8e6]/80 px-5 py-4 md:px-6 md:py-5">
        ...
        {hasMembership ? (<div className="flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-700">
            <Sparkles className="h-3.5 w-3.5"/>
            Active member
          </div>) : (<span className="rounded-full bg-[#e03a28]/10 px-3 py-1.5 text-xs font-bold text-[#e03a28]">
            Free · 1 year
          </span>)}
      </div>
      ...
      <div className="grid lg:grid-cols-[1.05fr_0.95fr] lg:items-stretch">
        ...
        <div className="border-b border-line/60 bg-gradient-to-b from-[#f4f8fb] to-white p-5 md:p-6 lg:border-b-0 lg:border-r">
          <div className="mb-4 flex items-center justify-between gap-2">
            ...
            {hasMembership && cardQuery.data ? (<span className="rounded-full bg-teal/10 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-teal">
                {cardQuery.data.flatDiscountPercent}% member discount
              </span>) : null}
          </div>

          {hasMembership && cardQuery.data ? (<div className="space-y-5">
              <div className="mx-auto w-full max-w-[400px] rounded-[28px] bg-gradient-to-b from-teal/5 to-transparent p-1 pt-2">
                <HealthIdCard data={cardQuery.data} flipped={flipped} onFlipChange={setFlipped} disableTapFlip hideBuiltInFlipHint className="mx-auto w-full"/>
              </div>
              <HomeCardActions cardData={cardQuery.data} flipped={flipped} onFlip={() => setFlipped((f) => !f)} onEdit={() => onEditOpen(true)} hasAddress={hasAddress}/>
              ...
              <HomeActivatePanel compact onActivate={plan ? () => subscribe.mutate(plan.id) : undefined} activatePending={subscribe.isPending}/>
            </div>)}
        </div>
        ...
        <div className="flex flex-col bg-white p-5 md:p-6">
          <div className="mb-4 flex items-center justify-between gap-2">
            ...
            <Link to="/wallet" className="flex items-center gap-0.5 text-xs font-semibold text-teal transition hover:underline">
              Manage wallet
