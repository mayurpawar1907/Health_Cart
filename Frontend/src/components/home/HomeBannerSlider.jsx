...
import { ArrowRight, BadgePercent, Bell, ChevronLeft, ChevronRight, Clock, FileText, FlaskConical, Gift, HeartPulse, Home, MessageCircle, Package, Receipt, Sparkles, Star, Truck, Users, Wallet, Zap, } from 'lucide-react';
import { cn } from '@/utils/utils';
import { useCatalogStats, usePlatformPricing } from '@/hooks/usePlatformPricing';
const SLIDE_THEMES = {
    teal: {
...
function SlideVisual({ type, theme, discountPct, packageCount, familyMembers }) {
    if (type === 'discount') {
        return (<div className="relative flex h-[140px] w-[140px] items-center justify-center md:h-[168px] md:w-[168px]">
    ...
    if (type === 'packages') {
        return (<div className="relative h-[140px] w-[160px] md:h-[168px] md:w-[180px]">
        {['Diabetic', 'Cardiac', 'PCOD', 'Senior'].map((label, i) => (<div key={label} className={cn('absolute rounded-2xl border border-white/20 bg-white/10 px-3 py-2 text-[10px] font-bold text-white backdrop-blur-sm', i === 0 && 'left-0 top-0 rotate-[-6deg]', i === 1 && 'right-0 top-2 rotate-[4deg]', i === 2 && 'bottom-8 left-2 rotate-[3deg]', i === 3 && 'bottom-0 right-0 rotate-[-3deg]')}>
            {label}
          </div>))}
    ...
    if (type === 'wallet') {
        return (<div className="relative flex h-[140px] w-[150px] flex-col items-center justify-end md:h-[168px] md:w-[170px]">
        {[0, 1, 2].map((i) => (<div key={i} className={cn('absolute rounded-2xl border border-white/25 bg-gradient-to-br from-white/25 to-white/5 backdrop-blur-sm', i === 0 && 'bottom-0 h-14 w-28', i === 1 && 'bottom-4 h-12 w-24 opacity-80', i === 2 && 'bottom-8 h-10 w-20 opacity-60')}/>))}
    ...
    return (<div className="relative flex h-[140px] w-[160px] items-center justify-center md:h-[168px] md:w-[180px]">
      <div className="absolute inset-0 rounded-full bg-white/5"/>
      {[0, 1, 2, 3, 4].map((i) => (<div key={i} className={cn('absolute grid h-10 w-10 place-items-center rounded-full bg-white/15 text-xs font-bold text-white ring-2 ring-white/20', i === 0 && 'left-1/2 top-0 -translate-x-1/2', i === 1 && 'left-0 top-1/2 -translate-y-1/2', i === 2 && 'right-0 top-1/2 -translate-y-1/2', i === 3 && 'bottom-2 left-4', i === 4 && 'bottom-2 right-4')}>
          {i === 0 ? 'You' : '•'}
        </div>))}
...
export function HomeBannerSlider() {
    ...
    const discountPct = isPromoActive ? activePercent : catalog.flatDiscount;
    const featured = useMemo(() => [
        {
            id: 'rates',
            tag: 'Launch offer',
            title: `Extra ${discountPct}% off at every payment`,
            highlight: `${catalog.testCount || '…'}+ tests · special partner rates`,
        ...
        },
        {
            id: 'packages',
            tag: 'Bundle & save',
            title: `${catalog.packageCount || '…'} health packages`,
            highlight: 'Full Body Basic and more',
        ...
        },
        {
            ...
            body: `Add ${catalog.familyMembers} members · WhatsApp reports in ${catalog.reportHours} hours · lifetime digital vault.`,
            cta: 'Manage card',
            to: '/home#healthid-card-section',
            visual: 'family',
            theme: SLIDE_THEMES.emerald,
        },
    ], [discountPct, catalog.testCount, catalog.packageCount, catalog.familyMembers, catalog.reportHours]);
    ...
    return (<section aria-label="Offers and updates" className="space-y-4">
      {/* Section header */}
      <div className="flex flex-wrap items-end justify-between gap-3 px-0.5">
        ...
        <Link to="/tests" className="inline-flex items-center gap-1.5 rounded-full border border-teal/25 bg-teal-light/50 px-4 py-2 text-xs font-bold text-teal transition hover:bg-teal-light">
          View all deals
      ...
      {/* Main carousel card */}
      <div className="overflow-hidden rounded-[28px] border border-line/80 bg-white shadow-[0_16px_48px_rgba(12,25,41,0.08)]" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
        <article key={animKey} className={cn('relative overflow-hidden bg-gradient-to-br text-white transition-opacity duration-500', slide.theme.shell)}>
          {/* Decorative layers */}
          ...
          <div className="relative grid min-h-[240px] items-center gap-6 p-6 md:min-h-[220px] md:grid-cols-[1fr_auto] md:p-8 lg:min-h-[200px]">
            <div className="min-w-0 space-y-3.5">
              ...
              <div className="flex flex-wrap items-center gap-3 pt-1">
                <Link to={slide.to} className="inline-flex items-center gap-2 rounded-xl bg-white px-5 py-2.5 text-sm font-bold text-ink shadow-[0_8px_24px_rgba(0,0,0,0.15)] transition hover:-translate-y-0.5 hover:bg-white/95">
                  {slide.cta}
            ...
            <div className="flex justify-center md:justify-end">
              <SlideVisual
                ...
                familyMembers={catalog.familyMembers}
              />
            </div>
          </div>
          ...
          <div className="relative border-t border-white/10 bg-black/10 px-4 py-3 backdrop-blur-sm md:px-6">
            <div className="flex items-center gap-3">
              <button type="button" aria-label="Previous offer" onClick={() => goTo((index - 1 + featured.length) % featured.length)} className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-white/15 text-white transition hover:bg-white/25">
                <ChevronLeft className="h-4 w-4"/>
              </button>
              <div className="flex flex-1 items-center gap-2">
                {featured.map((b, i) => (<button key={b.id} type="button" aria-label={`Show offer: ${b.title}`} onClick={() => goTo(i)} className="group relative h-1.5 flex-1 overflow-hidden rounded-full bg-white/20">
                    <span className={cn('absolute inset-y-0 left-0 rounded-full bg-white transition-all', i === index ? 'opacity-100' : 'w-0 opacity-0 group-hover:w-full group-hover:opacity-40')} style={i === index ? { width: `${progress}%` } : undefined}/>
                  </button>))}
              </div>
              <span className="hidden text-[10px] font-bold tabular-nums text-white/50 sm:inline">
                {index + 1}/{featured.length}
              </span>
              <button type="button" aria-label="Next offer" onClick={() => goTo((index + 1) % featured.length)} className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-white/15 text-white transition hover:bg-white/25">
        ...
        <div className="grid grid-cols-2 gap-px bg-line/60 sm:grid-cols-4">
          {featured.map((b, i) => (<button key={b.id} type="button" onClick={() => goTo(i)} className={cn('flex items-center gap-2.5 bg-white px-4 py-3 text-left transition hover:bg-teal-light/30', i === index && 'bg-teal-light/50 ring-1 ring-inset ring-teal/20')}>
              <span className={cn('grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-gradient-to-br text-white', i === index
                ? 'from-teal to-teal-dark shadow-sm'
                : 'from-slate-400/90 to-slate-500/90')}>
                {b.visual === 'discount' ? (<BadgePercent className="h-4 w-4"/>) : b.visual === 'packages' ? (<Package className="h-4 w-4"/>) : b.visual === 'wallet' ? (<Wallet className="h-4 w-4"/>) : (<Users className="h-4 w-4"/>)}
              </span>
      ...
      <div>
        ...
        <div className="-mx-1 flex gap-3 overflow-x-auto pb-1 px-1 scrollbar-thin">
          {UPDATES.map((item) => {
            ...
            return (<Link key={item.id} to={item.to} className={cn('group flex min-w-[220px] max-w-[260px] shrink-0 flex-col rounded-2xl border p-4 transition hover:-translate-y-0.5 hover:shadow-md sm:min-w-[240px]', item.tone)}>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          {QUICK_OFFERS.map((offer) => {
            ...
            return (<Link key={offer.id} to={offer.to} className="group relative overflow-hidden rounded-[22px] shadow-[0_8px_28px_rgba(12,25,41,0.1)] transition hover:-translate-y-1 hover:shadow-[0_16px_40px_rgba(12,25,41,0.14)]">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <Link to="/appointments/book" className="group relative overflow-hidden rounded-[22px] border border-teal/20 bg-gradient-to-r from-teal-light via-white to-cream p-5 transition hover:shadow-md">
        ...
        <Link to="/tests" className="group relative overflow-hidden rounded-[22px] border border-[#e03a28]/20 bg-gradient-to-r from-[#fff5f4] via-white to-teal-light/30 p-5 transition hover:shadow-md">
          ...
          <div className="relative flex items-center gap-4">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-[10px] font-bold uppercase tracking-wider text-[#e03a28]">{catalog.testCount || '…'}+ tests</p>
              <p className="font-display text-xl text-ink">Member special rates</p>
              <p className="text-sm text-ink-soft">
                Special partner rates + {discountPct}% at payment
              </p>
            </div>
