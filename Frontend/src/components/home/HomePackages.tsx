...
export function HomePackages() {
  ...
  if (packages.isLoading) return <Loading label="Loading health packages" />
  const list = packages.data ?? []
  if (!list.length) return null

  return (
    <section aria-labelledby="home-packages-heading">
      <div className="mb-5 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-soft">
            {PLATFORM.packageCount} official packages · Extra {PLATFORM.membership.flatDiscount}% off at payment · Free home collection
          </p>
          <div className="mt-2">
