...
export function HomeWalletPanel({ hasMembership }) {
    ...
    if (wallet.isLoading) {
        return (<div className="flex min-h-[320px] flex-1 items-center justify-center rounded-2xl border border-dashed border-line/80 bg-cream/30">
    ...
    return (<div className="flex flex-1 flex-col gap-4">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-teal via-[#256d8f] to-[#1a4d6d] p-5 text-white shadow-[0_12px_32px_rgba(26,77,109,0.25)]">
        ...
        <div className="relative flex items-start justify-between gap-3">
          ...
          {!hasMembership ? (<span className="flex items-center gap-1 rounded-full bg-white/15 px-2 py-1 text-[10px] font-bold uppercase text-white/80">
        ...
        {!hasMembership ? (<p className="relative mt-2 text-sm text-white/75">
            Activate card to unlock ₹{w.rules.JOINING_BONUS} joining bonus
          </p>) : (<p className="relative mt-2 text-sm text-white/75">
      ...
      <div className="rounded-xl border border-teal/20 bg-teal-light/20 p-4">
        ...
        <div className="mt-3 flex items-center gap-2">
          ...
          <button type="button" onClick={copyCode} className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-line/70 bg-white text-teal transition hover:border-teal/40 hover:bg-teal-light/30" aria-label="Copy referral code">
            {copied ? <Check className="h-4 w-4 text-emerald-600"/> : <Copy className="h-4 w-4"/>}
          </button>
          <button type="button" onClick={shareReferral} className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-line/70 bg-white text-teal transition hover:border-teal/40 hover:bg-teal-light/30" aria-label="Share referral">
