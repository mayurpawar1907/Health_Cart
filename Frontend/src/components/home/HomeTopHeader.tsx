...
export function HomeTopHeader({ firstName, city, pincode, unreadCount, hasMembership }: HomeTopHeaderProps) {
  ...
  return (
    <section className="overflow-hidden rounded-[24px] border border-line/60 shadow-[0_12px_40px_rgba(43,90,121,0.12)]">
      {/* Banner hero */}
          ...
          <div className="text-white">
            ...
            <div className="mt-5 flex flex-wrap items-center gap-3">
              <Link
                ...
                className="inline-flex items-center rounded-full bg-brand-red px-5 py-2.5 text-sm font-semibold text-white shadow-lg transition hover:bg-brand-red-dark"
              >
                Book home collection
              </Link>
              <Link
                to="/profile"
                className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3.5 py-2 backdrop-blur-sm transition hover:bg-white/15"
              >
      ...
      <div className="relative z-10 -mt-1 bg-white px-5 pb-5 pt-4 md:px-7 md:pb-6">
        <form onSubmit={onSearch} className="relative">
          ...
          <input
            ...
            placeholder="Search tests, packages, vitamins, diabetes..."
            className={cn(
              'w-full rounded-2xl border border-line bg-cream/40 py-4 pl-12 pr-28',
              'text-[15px] text-ink placeholder:text-ink-soft/70',
              'shadow-[0_4px_16px_rgba(43,90,121,0.06)]',
              'outline-none transition',
              'focus:border-teal focus:bg-white focus:shadow-[0_0_0_4px_rgba(43,90,121,0.1)]',
            )}
          />
          <button
            type="submit"
            className="absolute right-2 top-1/2 -translate-y-1/2 rounded-xl bg-teal px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-teal-dark active:scale-[0.98]"
          >
            Search
          </button>
        </form>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          ...
          {QUICK_SEARCHES.map((term) => (
            <button
              ...
              className="rounded-full border border-line bg-white px-3.5 py-1.5 text-xs font-medium text-ink-soft shadow-sm transition hover:border-teal/40 hover:bg-teal-light/70 hover:text-teal"
            >
              {term}
            </button>
          ))}
...
function MembershipPill({ compact }: { compact?: boolean }) {
  return (
    <Link
      to="/membership/card"
      className={cn(
        'inline-flex items-center gap-1.5 rounded-full bg-white/15 font-semibold text-white backdrop-blur-sm transition hover:bg-white/25',
...
function BannerThumb({ src, label }: { src: string; label: string }) {
  return (
