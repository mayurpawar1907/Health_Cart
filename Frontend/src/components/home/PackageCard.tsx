...
export function PackageCard({ pkg, accentIndex = 0, compact, showIncluded }: PackageCardProps) {
  const [expanded, setExpanded] = useState(false)
  const staticPkg = findStaticPackage(pkg)
  const included = packageIncludedTests(pkg)
  const mrp = staticPkg?.mrp ?? Number(pkg.price)
  const memberPrice = staticPkg?.specialPrice ?? Number(pkg.memberPrice ?? pkg.discountedPrice ?? pkg.price)
  ...
  return (
    <article
      className={cn(
        'group flex h-full flex-col overflow-hidden rounded-3xl border border-line/70 bg-white shadow-[0_8px_30px_rgba(12,25,41,0.06)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_16px_40px_rgba(12,25,41,0.1)]',
      ...
      <div className="flex flex-1 flex-col p-5">
        <div className="flex items-start justify-between gap-2">
          ...
          <div className="flex flex-wrap justify-end gap-1">
            {pkg.isPopular ? (
              <span className="rounded-full bg-teal px-2.5 py-1 text-[10px] font-bold text-white">Popular</span>
            ) : null}
        ...
        {showIncluded && included.length > 0 ? (
          <div className="mt-3 rounded-xl border border-line/70 bg-cream/40 p-3">
        ...
        <div className="mt-auto border-t border-line/60 pt-4">
          ...
          <div className="mt-4 flex gap-2">
            <Link
              to={`/appointments/book?testId=${pkg.id}`}
              className="flex-1 rounded-xl bg-gradient-to-r from-[#e03a28] to-[#c42e1e] py-2.5 text-center text-xs font-bold text-white shadow-sm transition hover:brightness-105"
            >
              Book package
            </Link>
            <Link
              to={`/tests/${pkg.slug}`}
              className="rounded-xl border border-line px-3 py-2.5 text-xs font-semibold text-ink-soft transition hover:border-teal hover:text-teal"
            >
              Details
            </Link>
          </div>
