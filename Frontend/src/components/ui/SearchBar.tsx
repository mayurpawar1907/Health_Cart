...
export function SearchBar({ placeholder = 'Search tests, health packages...' }: { placeholder?: string }) {
  ...
  return (
    <form onSubmit={onSubmit} className="relative">
      ...
      <input
        ...
        className="w-full rounded-full border border-line bg-white py-3.5 pl-12 pr-4 shadow-sm outline-none focus:border-teal focus:ring-4 focus:ring-teal/10"
      />
    </form>
  )
}

export function NotifyBell({ count, onDark, compact }: { count: number; onDark?: boolean; compact?: boolean }) {
  ...
  return (
    ...
    >
      ...
      {count > 0 ? (
        <span className="absolute -right-1 -top-1 grid h-[18px] min-w-[18px] place-items-center rounded-full bg-brand-red px-1 text-[10px] font-bold leading-none text-white ring-2 ring-white">
          {count > 9 ? '9+' : count}
        </span>
      ) : null}
    </Link>
