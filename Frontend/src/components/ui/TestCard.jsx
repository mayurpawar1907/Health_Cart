...
export function TestCard({ test }) {
    ...
    return (<article className="glass-panel flex h-full flex-col overflow-hidden rounded-3xl">
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-ink-soft">
          {test.isPackage ? (<span className="inline-flex items-center gap-1">
        ...
        <div className="mt-4 flex items-end gap-2">
          ...
          {showStrike ? (<span className="pb-0.5 text-sm text-ink-soft line-through">{formatMoney(mrp)}</span>) : null}
        </div>
