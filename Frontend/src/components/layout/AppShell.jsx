...
import { CalendarDays, FlaskConical, History, Home, LogOut, Plus, UserRound, Wallet, } from 'lucide-react';
...
function SidebarLink({ item }) {
    ...
    return (<NavLink to={item.to} end={item.end} className={({ isActive }) => cn('flex items-center gap-2.5 rounded-lg px-2.5 py-2 text-[13px] font-medium transition-colors', isActive ? 'bg-teal-light text-teal' : 'text-ink-soft hover:bg-slate-50 hover:text-ink')}>
...
export function AppShell() {
    ...
    const unread = useQuery({
        ...
        queryFn: async () => unwrap((await api.get('/notifications/unread-count')).data),
        staleTime: 30000,
    ...
    return (<div className="flex h-screen w-screen overflow-hidden app-mesh">
      ...
      <aside className="hidden h-full w-[220px] shrink-0 flex-col border-r border-line/80 bg-white lg:flex xl:w-[232px]">
        <div className="flex h-full flex-col px-3 py-4">
          ...
          <NavLink to="/appointments/book" className="mt-5 flex items-center justify-center gap-1.5 rounded-lg bg-brand-red py-2.5 text-xs font-bold text-white transition hover:bg-brand-red-dark">
          ...
          <div className="mt-auto space-y-2 border-t border-line/70 pt-3">
            {isAdmin ? (<NavLink to="/admin" className="block rounded-lg px-2.5 py-1.5 text-[11px] font-semibold text-ink-soft transition hover:bg-slate-50 hover:text-teal">
                Admin console →
              </NavLink>) : null}
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <header className="flex h-12 shrink-0 items-center justify-between gap-3 border-b border-line/70 bg-white px-4 lg:h-[52px] lg:px-6">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            {wallet.data ? (<NavLink to="/wallet" className="inline-flex items-center gap-1.5 rounded-lg bg-teal-light/80 px-2.5 py-1.5 text-xs font-bold text-teal">
                <Wallet className="h-3.5 w-3.5"/>
                <span className="hidden sm:inline">Wallet</span>
                {formatMoney(wallet.data.totalSpendable)}
              </NavLink>) : null}
            {isAdmin ? (<NavLink to="/admin" className="hidden rounded-lg px-2.5 py-1.5 text-xs font-semibold text-ink-soft hover:text-teal sm:inline-flex">
                Admin
              </NavLink>) : null}
            <NotifyBell count={unread.data?.count ?? 0} compact/>
            <NavLink to="/profile" title="Profile & settings" className="inline-flex items-center gap-2 rounded-lg border border-line/80 bg-white py-1 pl-1 pr-2.5 transition hover:border-teal/30">
        ...
        <div className="flex items-end justify-around rounded-xl border border-line/80 bg-white px-0.5 py-1.5 shadow-[0_4px_20px_rgba(12,25,41,0.1)]">
          {mobileNav.map((item) => {
            ...
            if ('fab' in item && item.fab) {
                return (<NavLink key={item.to} to={item.to} className="-mt-4 grid h-11 w-11 place-items-center rounded-xl bg-brand-red text-white shadow-md transition active:scale-95">
