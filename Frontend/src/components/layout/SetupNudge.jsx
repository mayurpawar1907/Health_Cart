...
export function SetupNudge({ hasMembership, hasAddress }) {
    const [dismissed, setDismissed] = useState(false);
    if (dismissed || (hasMembership && hasAddress))
        return null;
    const steps = [
        !hasMembership && {
    ...
    return (<div className="glass-panel relative rounded-3xl p-5 md:p-6">
      <button type="button" aria-label="Dismiss" onClick={() => setDismissed(true)} className="absolute right-4 top-4 rounded-xl p-1.5 text-ink-soft transition hover:bg-white/80">
        <X className="h-4 w-4"/>
      </button>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        {steps.map((s) => (<div key={s.title} className="rounded-2xl border border-line/60 bg-white/60 p-4">
