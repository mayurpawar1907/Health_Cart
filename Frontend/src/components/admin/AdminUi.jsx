...
export function StatCard({ label, value, hint, tone = 'default' }) {
    return (<div className={cn('glass-panel rounded-3xl p-5 transition hover:shadow-md', tone === 'accent' && 'ring-1 ring-brand-red/15', tone === 'warn' && 'ring-1 ring-amber-300/40')}>
      <p className="text-[11px] font-bold uppercase tracking-wider text-ink-soft">{label}</p>
      <p className="mt-2 font-display text-3xl text-ink">{value}</p>
      {hint ? <p className="mt-1 text-xs text-ink-soft">{hint}</p> : null}
    </div>);
}
export function AdminSection({ title, subtitle, action, children }) {
    return (<section>
...
export function AdminPanel({ title, subtitle, children }) {
    return (<div className="glass-panel rounded-3xl p-5 md:p-6">
...
export function AdminToolbar({ children }) {
    return (<div className="glass-panel flex flex-wrap items-center gap-3 rounded-2xl p-3 md:p-4">
      {children}
    </div>);
}
export function AdminSearch({ value, onChange, placeholder = 'Search…', className, }) {
    return (<div className={cn('relative min-w-[200px] flex-1', className)}>
      <Search className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-ink-soft"/>
      <input type="search" value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="w-full rounded-xl border border-line/80 bg-white/90 py-2.5 pl-10 pr-4 text-sm outline-none transition focus:border-teal focus:shadow-[0_0_0_4px_rgba(26,77,109,0.08)]"/>
    </div>);
}
export function AdminSelect({ value, onChange, children, className, label, }) {
    const select = (<select value={value} onChange={(e) => onChange(e.target.value)} className={cn('rounded-xl border border-line/80 bg-white/90 px-4 py-2.5 text-sm font-medium shadow-sm outline-none transition focus:border-teal focus:shadow-[0_0_0_4px_rgba(26,77,109,0.08)]', className)}>
      {children}
    </select>);
    if (!label)
        return select;
...
export function AdminPagination({ page, pages, onPage, }) {
    if (pages <= 1)
        return null;
...
export function AdminQuickLink({ to, label }) {
