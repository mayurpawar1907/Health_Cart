...
function SidebarNav({ isSuper, onNavigate }: { isSuper: boolean; onNavigate?: () => void }) {
    ...
    <div className="flex h-screen w-screen overflow-hidden app-mesh">
      ...
      <aside className="hidden h-full w-[220px] shrink-0 flex-col border-r border-line/80 bg-white lg:flex xl:w-[232px]">
        <div className="flex h-full flex-col px-3 py-4">
          ...
          <NavLink
            to="/home"
            className="mt-4 flex items-center justify-center gap-1.5 rounded-lg border border-line/80 bg-white py-2.5 text-xs font-bold text-teal transition hover:border-teal/30 hover:bg-teal-light/40"
          >
          ...
          <div className="mt-auto space-y-2 border-t border-line/70 pt-3">
            <span
              className={cn(
                'mx-1 inline-flex rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide',
                isSuper ? 'bg-brand-red-light text-brand-red' : 'bg-teal-light text-teal-dark',
              )}
            >
              {user?.role?.replace('_', ' ')}
            </span>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-12 shrink-0 items-center justify-between gap-3 border-b border-line/70 bg-white px-4 lg:h-[52px] lg:px-6">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            <NavLink
              to="/home"
              className="hidden rounded-lg px-2.5 py-1.5 text-xs font-semibold text-ink-soft transition hover:text-teal sm:inline-flex"
            >
              Customer app
            </NavLink>
            <span
            ...
            <span
              title="Profile"
              className="inline-flex items-center gap-2 rounded-lg border border-line/80 bg-white py-1 pl-1 pr-2.5"
            >
            ...
            if ('external' in item && item.external) {
              return (
                <NavLink
                  key={item.to}
                  to={item.to}
                  className="-mt-4 grid h-11 w-11 place-items-center rounded-xl bg-teal text-white shadow-md transition active:scale-95"
                >
