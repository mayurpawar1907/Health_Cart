...
import { cn, formatDate, formatMoney } from '@/utils/utils';
...
export function MembershipPage() {
    ...
    const perks = [
    { icon: ShieldCheck, title: `Special rates on ${stats.testCount || '…'}+ tests`, body: 'Official partner pricing from your lab catalog.' },
    { icon: Truck, title: `${stats.packageCount || '…'} diagnostic packages`, body: 'Full Body, Diabetic, Cardiac, PCOD, Cancer panels & more.' },
    { icon: Users, title: 'Family on one card', body: `Add spouse, parents, and children — up to ${stats.familyMembers} members.` },
    { icon: MessageCircle, title: 'WhatsApp updates', body: 'Bookings, reminders, and reports on WhatsApp.' },
];
    const qc = useQueryClient();
    const current = useQuery({
        queryKey: ['membership'],
        queryFn: async () => unwrap((await api.get('/membership')).data),
    });
    ...
    const subscribe = useMutation({
        mutationFn: async (planId) => api.post('/membership', { planId }),
        onSuccess: () => {
    ...
    const addFamily = useMutation({
        mutationFn: async (payload) => api.post('/family', payload),
        onSuccess: () => {
    ...
    return (<UserPage>
      ...
      {!m && (<UserPanel title="Get your free card" subtitle={plan?.description}>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {perks.map((p) => (<div key={p.title} className="rounded-2xl border border-line/60 bg-white/60 p-4">
                <p.icon className="mb-2 h-5 w-5 text-teal"/>
                <p className="font-semibold">{p.title}</p>
                <p className="mt-1 text-sm text-ink-soft">{p.body}</p>
              </div>))}
          </div>
          {plan ? (<Button className="mt-6 rounded-xl" disabled={subscribe.isPending} onClick={() => subscribe.mutate(plan.id)}>
      ...
      {m ? (<>
          ...
          <UserPanel title="Family on your card" subtitle={`Up to ${m.plan.maxFamilyMembers ?? 5} members on one card`}>
            <div className="mt-4 space-y-2">
              {(m.members ?? []).map((member) => (<div key={member.id} className="flex items-center justify-between rounded-2xl border border-line px-4 py-3">
                  <div>
            ...
            <div className="mt-6 grid gap-3 md:grid-cols-4">
              ...
              <div className="flex items-end">
                <Button className="w-full" disabled={!name || addFamily.isPending} onClick={() => addFamily.mutate({ name, relation, age: age ? Number(age) : undefined })}>
                  Add member
                </Button>
              </div>
            </div>
            {(family.data ?? []).length > (m.members?.length ?? 0) ? (<p className="mt-3 text-xs text-ink-soft">Saved family members are automatically linked to your card.</p>) : null}
          </UserPanel>
        </>) : null}
    </UserPage>);
}
export function MembershipCardPage() {
    const q = useQuery({
        queryKey: ['card'],
        queryFn: async () => unwrap((await api.get('/membership/card')).data),
    });
    if (q.isLoading || !q.data)
        return <Loading />;
    const c = q.data;
    return (<UserPage className="max-w-lg">
      <div data-print-hide>
      ...
      <UserPanel title="Card details" className="print:hidden">
        ...
        <div className="mt-5 border-t border-line pt-5">
          ...
          <ul className="mt-3 space-y-2">
            {c.familyMembers.map((m) => (<li key={m.name} className="flex items-center justify-between text-sm">
