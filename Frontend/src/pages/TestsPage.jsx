...
import { TestsPageHero } from '@/components/tests/TestsPageHero';
...
export function TestsPage() {
    ...
    const tests = useQuery({
        ...
        queryFn: async () => unwrap((await api.get('/tests', {
            params: {
    ...
    return (<UserPage className="space-y-8">
      <TestsPageHero view={view} onViewChange={setView} search={search} onSearchChange={setSearch} resultCount={list.length}/>

      <PaymentDiscountBanner />

      {!packagesOnly && view === 'all' ? (<TestsCategoryGrid categories={cats.data ?? []} activeSlug={category} onSelect={setCategoryFilter}/>) : null}
      ...
      <section aria-label="Catalog results">
        <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
          ...
          {!packagesOnly ? (<div className="flex items-center gap-2">
