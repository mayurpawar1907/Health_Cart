...
export function LoginPage() {
    ...
    return (<AuthFrame title="Welcome back" subtitle="Sign in with your email or mobile to continue.">
      {registered ? (<p className="mb-4 rounded-2xl bg-teal-light px-4 py-3 text-sm text-teal-dark">
...
export function AuthFrame({ title, subtitle, children }) {
    return (<div className="grid h-screen w-screen overflow-hidden app-mesh lg:grid-cols-2">
      <div className="relative hidden h-full overflow-hidden lg:flex lg:flex-col lg:justify-between">
        ...
        <div className="relative max-w-lg px-12 pb-4">
          ...
          <div className="mt-8 grid grid-cols-3 gap-4">
            {['63+ tests', 'Free collection', 'Family card'].map((t) => (<div key={t} className="rounded-2xl border border-white/10 bg-white/5 px-3 py-3 text-center text-xs font-semibold text-white/80 backdrop-blur-sm">
                {t}
              </div>))}
          </div>
        </div>
