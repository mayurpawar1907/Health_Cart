...
export function AdminMembershipsPage() {
    ...
    const memberships = useQuery({
        ...
        queryFn: async () => unwrap((await api.get('/admin/memberships')).data),
    });
    const plans = useQuery({
        queryKey: ['admin-plans'],
        queryFn: async () => unwrap((await api.get('/admin/membership-plans')).data),
    });
    const users = useQuery({
        queryKey: ['admin-users-options'],
        queryFn: async () => unwrap((await api.get('/admin/users', { params: { role: 'USER', limit: 100 } })).data),
    });
    ...
    if (memberships.isLoading)
        return <Loading label="Loading memberships"/>;
    return (<AdminPage>
      ...
      <AdminPanel title="Grant membership" subtitle="Activate a HealthID Card for a customer">
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <AdminSelect label="Plan" value={planId} onChange={setPlanId}>
            ...
            {(plans.data ?? []).map((p) => (<option key={p.id} value={p.id}>{p.name} ({p.durationDays} days)</option>))}
          </AdminSelect>
        </div>
      ...
      <AdminTable>
        <table className="w-full min-w-[800px] text-left text-sm">
          ...
          <tbody>
            {(memberships.data ?? []).map((m) => (<AdminTr key={m.id}>
