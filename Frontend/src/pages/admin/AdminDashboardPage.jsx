...
import { AdminPage, AdminPageHeader, AdminPanel, AdminQuickLink, AdminSection, AdminTHead, AdminTh, AdminTr, AdminTd, Badge, StatCard, AdminTable, } from '@/components/admin/AdminUi';
export function AdminDashboardPage() {
    const { data, isLoading } = useQuery({
        queryKey: ['admin-dashboard'],
        queryFn: async () => unwrap((await api.get('/admin/dashboard')).data),
    });
    if (isLoading)
    ...
    return (<AdminPage>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <AdminSection title="Recent appointments" action={<Link to="/admin/appointments" className="text-sm font-semibold text-teal hover:underline">View all</Link>}>
            <AdminTable>
              <table className="w-full min-w-[600px] text-left text-sm">
                ...
                <tbody>
                  {data.recentAppointments.map((a) => (<AdminTr key={a.id}>
