...
import { AdminPage, AdminPageHeader, AdminPanel, AdminSection, AdminTHead, AdminTh, AdminTr, AdminTd, AdminTable, Badge, } from '@/components/admin/AdminUi';
...
export function AdminCustomerDetailPage() {
    ...
    const { data, isLoading } = useQuery({
        ...
        enabled: !!id,
        queryFn: async () => {
            const u = unwrap((await api.get(`/admin/users/${id}`)).data);
    ...
    return (<AdminPage>
      <AdminPageHeader title={data.fullName} subtitle={data.email} actions={<Link to="/admin/customers">
        ...
        <AdminPanel title="Profile">
          ...
          <div className="mt-6 border-t border-line/60 pt-5">
            ...
            <ul className="mt-3 space-y-2 text-sm">
              {data.addresses.map((a) => (<li key={a.id} className="rounded-xl bg-teal-light/40 p-3">
          ...
          <div className="mt-6 border-t border-line/60 pt-5">
            ...
            <ul className="mt-3 space-y-1.5 text-sm">
              {data.familyMembers.map((f) => (<li key={f.id} className="rounded-lg bg-white/60 px-3 py-2">{f.name} · {f.relation}{f.age ? ` · ${f.age}y` : ''}</li>))}
          ...
          <AdminSection title="Memberships">
            <AdminTable>
              <table className="w-full text-left text-sm">
                ...
                <tbody>
                  {data.memberships.map((m) => (<AdminTr key={m.id}>
                  ...
                  {!data.memberships.length ? (<tr><td colSpan={4} className="px-5 py-8 text-center text-sm text-ink-soft">No memberships</td></tr>) : null}
                </tbody>
              </table>
            </AdminTable>
          </AdminSection>
          ...
          <AdminSection title="Appointments">
            <AdminTable>
              <table className="w-full text-left text-sm">
                ...
                <tbody>
                  {data.appointments.map((a) => (<AdminTr key={a.id}>
                  ...
                  {!data.appointments.length ? (<tr><td colSpan={5} className="px-5 py-8 text-center text-sm text-ink-soft">No appointments</td></tr>) : null}
                </tbody>
              </table>
            </AdminTable>
          </AdminSection>
