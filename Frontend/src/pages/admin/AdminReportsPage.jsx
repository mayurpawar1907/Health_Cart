...
import { AdminPage, AdminPageHeader, AdminPanel, AdminSelect, AdminTHead, AdminTh, AdminTr, AdminTd, AdminTable, Badge, } from '@/components/admin/AdminUi';
export function AdminReportsPage() {
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: async () => unwrap((await api.get('/admin/reports')).data),
    });
    const appointments = useQuery({
        queryKey: ['admin-appts-for-reports'],
        queryFn: async () => unwrap((await api.get('/admin/appointments')).data),
    });
    const upload = useMutation({
        mutationFn: async () => {
            if (!file || !appointmentId)
                throw new Error('Missing file or appointment');
    ...
    return (<AdminPage>
      ...
      <AdminPanel title="Upload report file" subtitle="PDF, PNG, JPG or TXT · max 10 MB · triggers notification + WhatsApp">
        <div className="grid gap-4 sm:grid-cols-2">
          <AdminSelect label="Booking" value={appointmentId} onChange={setAppointmentId} className="w-full">
            <option value="">Select appointment…</option>
            {(appointments.data ?? []).map((a) => (<option key={a.id} value={a.id}>
          ...
          <div className="sm:col-span-2">
            ...
            <label className="flex cursor-pointer flex-wrap items-center gap-3 rounded-2xl border border-dashed border-teal/40 bg-teal-light/20 px-4 py-4 transition hover:bg-teal-light/35">
              ...
              <input ref={fileRef} type="file" accept=".pdf,.png,.jpg,.jpeg,.webp,.txt,application/pdf,image/*,text/plain" className="hidden" onChange={(e) => setFile(e.target.files?.[0] ?? null)}/>
            </label>
          </div>
        </div>
        <Button size="sm" className="mt-4 rounded-xl" disabled={!appointmentId || !file || upload.isPending} onClick={() => upload.mutate()}>
          <Upload className="mr-2 h-4 w-4"/>
          {upload.isPending ? 'Uploading…' : 'Upload & release report'}
        </Button>
        {upload.isError ? (<p className="mt-2 text-sm text-brand-red">Upload failed. Check file type and size.</p>) : null}
      </AdminPanel>
        ...
        <table className="w-full min-w-[900px] text-left text-sm">
          ...
          <tbody>
            {(data ?? []).map((r) => (<AdminTr key={r.id}>
                ...
                <AdminTd>
                  {r.fileUrl ? (<Button size="sm" variant="secondary" className="rounded-lg text-xs" onClick={() => downloadAuthenticatedFile(`/admin/reports/${r.id}/file`, r.fileName ?? 'report.pdf')}>
                      <Download className="mr-1 h-3.5 w-3.5"/> Download
                    </Button>) : (<span className="text-xs text-ink-soft">No file</span>)}
                </AdminTd>
              </AdminTr>))}
          </tbody>
        </table>
