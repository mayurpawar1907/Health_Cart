...
export function TestDetailsPage() {
    ...
    const q = useQuery({
        queryKey: ['test', id],
        queryFn: async () => unwrap((await api.get(`/tests/${id}`)).data),
        staleTime: 0,
    ...
    if (q.isLoading || !q.data)
        return <Loading />;
    const t = q.data;
    const memberPrice = t.memberPrice ?? t.discountedPrice ?? t.price;
    const displayMrp = t.price;
    const includedNames = t.isPackage ? packageIncludedTests(t) : [];
    const included = t.packageTests?.map((pt) => pt.includedTest) ??
    ...
    return (<UserPage>
      <div className="grid gap-6 lg:grid-cols-[1.4fr_0.8fr]">
        <div className="space-y-5">
          ...
          {t.isPackage && included.length > 0 ? (<Card className="p-5">
              <h2 className="flex items-center gap-2 font-display text-xl">
                <Layers className="h-5 w-5 text-teal"/>
                Tests included in this package
              </h2>
              <ul className="mt-4 space-y-2">
                {included.map((item) => (<li key={item.id} className="flex items-start gap-2 rounded-xl bg-teal-light/40 px-4 py-3 text-sm">
          ...
          {!t.isPackage ? (<Card className="p-5">
              <h2 className="font-display text-xl">Parameters included</h2>
              <ul className="mt-3 grid gap-2 md:grid-cols-2">
                {t.parameters.map((p) => (<li key={p.id} className="rounded-2xl bg-cream px-3 py-2 text-sm">
                    {p.name}
                  </li>))}
              </ul>
            </Card>) : null}

          {t.faqs?.length ? (<Card className="space-y-3 p-5">
              <h2 className="font-display text-xl">FAQs</h2>
              {t.faqs.map((f) => (<div key={f.q}>
        ...
        <Card className="h-fit p-6">
          ...
          <div className="mt-4 flex items-end gap-2">
            ...
            {Number(memberPrice) < Number(displayMrp) ? (<span className="text-ink-soft line-through">{formatMoney(Number(displayMrp))}</span>) : null}
          </div>
          {t.membershipFree ? (<p className="mt-2 text-sm text-ink-soft">Included free with your HealthID Card</p>) : (<PaymentDiscountCardNote specialPrice={Number(memberPrice)}/>)}
