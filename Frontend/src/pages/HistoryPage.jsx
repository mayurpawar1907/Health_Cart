...
import { UserBadge, UserEmptyRow, UserPage, UserPageHeader, UserPanel, UserSearch, UserTable, UserTd, UserTh, UserTHead, UserToolbar, UserTr, } from '@/components/user/UserUi';
export function HistoryPage() {
    ...
    const q = useQuery({
        queryKey: ['history'],
        queryFn: async () => unwrap((await api.get('/history')).data),
    });
    const reportQuery = useQuery({
        queryKey: ['report', reportId],
        enabled: !!reportId,
        queryFn: async () => unwrap((await api.get(`/reports/${reportId}`)).data),
    });
    useEffect(() => {
        if (reportQuery.data)
            setActiveReport(reportQuery.data);
    }, [reportQuery.data]);
    const filtered = useMemo(() => {
        ...
        if (!qLower)
            return list;
        return list.filter((a) => a.test.name.toLowerCase().includes(qLower) ||
    ...
    return (<UserPage>
      ...
      {activeReport ? (<UserPanel title="Lab report">
          ...
          {activeReport.fileUrl ? (<Button className="mt-4 rounded-xl" onClick={() => downloadAuthenticatedFile(`/reports/${activeReport.id}/file`, activeReport.fileName ?? 'lab-report.pdf')}>
      ...
      <UserTable>
        <table className="w-full min-w-[880px] text-left text-sm">
          ...
          <tbody>
            {!filtered.length ? (<UserEmptyRow colSpan={7} message="No records match your search"/>) : (filtered.map((a) => {
            const report = a.reports?.[0];
            const status = report?.status ?? 'PENDING';
            return (<UserTr key={a.id}>
                    <UserTd>
                      <p className="font-semibold">{a.test.name}</p>
                      {report?.summary && status === 'AVAILABLE' ? (<p className="mt-0.5 line-clamp-1 text-xs text-ink-soft">{report.summary}</p>) : null}
                    </UserTd>
                    ...
                    <UserTd>
                      {status === 'AVAILABLE' && report ? (<Button size="sm" className="rounded-lg text-xs" onClick={() => {
                        setParams({ report: report.id });
                        setActiveReport(report);
                    }}>
                          View report
                        </Button>) : (<span className="text-xs text-ink-soft">Processing…</span>)}
                    </UserTd>
                  </UserTr>);
        }))}
          </tbody>
